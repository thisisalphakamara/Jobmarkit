...
const JobListing = () => {
  ...
  useEffect(() => {
    const filterJobs = () => {
      ...
      const matchesSearchLocation = (job) => {
        ...
        // Check against town, district, or province
        return (
          jobLoc.town.toLowerCase().includes(searchLoc) ||
          jobLoc.district.toLowerCase().includes(searchLoc) ||
          jobLoc.province.toLowerCase().includes(searchLoc)
        );
      };
  ...
  const getMatchColor = (percentage) => {
    if (percentage >= 90) return "text-gray-700 bg-gray-100 border-gray-200";
    if (percentage >= 80) return "text-gray-700 bg-gray-100 border-gray-200";
    if (percentage >= 70) return "text-gray-600 bg-gray-50 border-gray-200";
    if (percentage >= 60) return "text-gray-500 bg-gray-50 border-gray-200";
                          ...
                          <div
                            ...
                            className={`bg-white rounded-lg p-3 border transition-colors cursor-pointer ${
                              selectedAIRecommendation?._id === job._id
                                ? "border-gray-700 bg-gray-50 ring-2 ring-gray-200"
