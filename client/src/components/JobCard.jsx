...
const JobCard = ({
  ...
  const handleAudioNarration = () => {
    ...
    if (preferredLanguage === "krio") {
      ...
      const narrationText = `Dis nah woke way ${company} post. This woke dae look for person way nah ${title}. Di woke dae nah ${location}. Di salary nah ${salaryForSpeech} for month. Dis nah ${level} position pa ${category} woke. Di description for this woke nah: ${description}. Dem post dis job ${timePosted}.`;
      ...
      const salaryForSpeech =
        salary.replace(/Le\s*/g, "").replace(/,/g, "") + " leones";
      const narrationText = `This is a job posted by ${company}. The job title is ${title}. The job is located in ${location}. The salary is ${salaryForSpeech} per month. This is a ${level} position in ${category}. The job description is: ${description}. This job was posted ${timePosted}.`;
  ...
  const handleQuickApply = async () => {
    ...
    if (showQuickApply) {
      // Quick Apply mode - apply directly
      setIsApplying(true);
      try {
        const token = await getToken();
        const response = await axios.post(
      ...
      // Normal mode - navigate to apply page
  ...
  const getJobTitle = () => {
    return `${job.title} at ${job.companyId?.name || "Company"}`;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`p-6 pb-4 pr-6 ${viewMode === "list" ? "flex-1" : ""}`}>
        ...
        <div className={`px-6 pb-4 ${viewMode === "list" ? "flex-1" : ""}`}>
          ...
          <div className="mb-4 max-h-60 overflow-y-auto p-1 bg-white rounded border border-gray-100">
            ...
            >
              {stripHtmlTags(getDisplayDescription())}
            </p>
          </div>
